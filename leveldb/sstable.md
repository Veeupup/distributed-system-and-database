[leveldb sstable](https://leveldb-handbook.readthedocs.io/zh/latest/sstable.html) 这篇讲leveldb中sstable的文章确实不错，但刚开始看仍然会很懵。  

<img src="https://leveldb-handbook.readthedocs.io/zh/latest/_images/sstable_logic.jpeg" width="40%" />  
...
有个疑问是为什么要弯弯绕绕搞个meta index block，直接把filter block的位置存在footer block不就OK了吗？注意到meta index block存储了filter block的位置时也是按key value的方式存储的，难道说可能有多个filter block？不同的方式作为filter？这一块没细看，回头补。    
